...
Let:
- $N(T, K, W)$ = Population count for Team $T$, Kind $K$, Week $W$.
- $V(T, K, W)$ = Visibility of the primitive cell $(T, K, W)$.
- $V_{Team}(T, W)$ = Visibility of the team-wide average for week $W$.
- $V_{Global}(K, W)$ = Visibility of the global average for kind $K$, week $W$.

### A. Weekly Base Primitive (Team-Kind)
A cell $(T, K, W)$ is visible if and only if **all** kinds for that team in that week meet the threshold.
$$V(T, K, W) = \bigwedge_{k \in Kinds} (N(T, k, W) \ge 3)$$
...
A team total for week $W$ is visible if the aggregate team size meets the threshold.
$$V_{Team}(T, W) = (\sum_{k \in Kinds} N(T, k, W) \ge 3)$$
...
A global average for kind $K$ is visible if the "Hidden Union" (sum of counts of all suppressed team-kind cells for that kind) is either zero or $\ge 3$.
$$V_{Global}(K, W) = (\sum_{T: \neg V(T, K, W)} N(T, K, W) \ge 3) \lor (\forall T, V(T, K, W))$$
...
$$V_{Period}(Scope, Kind, \text{Weeks}) = \bigwedge_{w \in \text{Weeks}} V_{Weekly}(Scope, Kind, w)$$
...
1.  **Attribute Disclosure**: While values are hidden, the *fact* of suppression reveals that the population $N < 3$. In small organizations, this itself may be sensitive information (e.g., revealing that only 1-2 people in a specific team are active).
...
**Condition**: *For any aggregate result $A$ derived from a set of records $R$, $A$ is published only if the set of records $R \setminus \bigcup (\text{Visible Sub-aggregates of } A)$ is either empty or has a cardinality $|R'| \ge K$ (where $K=3$).*
