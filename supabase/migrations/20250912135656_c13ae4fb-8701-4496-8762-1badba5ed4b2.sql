-- Create TipoAusencia enum if it doesn't exist
DO $$ BEGIN
    CREATE TYPE "TipoAusencia" AS ENUM ('DAYOFF', 'FERIAS', 'LICENCA_MEDICA');
EXCEPTION
    WHEN duplicate_object THEN 
        -- If enum exists, add the new value
        ALTER TYPE "TipoAusencia" ADD VALUE IF NOT EXISTS 'LICENCA_MEDICA';
END $$;
...
CREATE TABLE IF NOT EXISTS public.team_capacity_alerts (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
DROP POLICY IF EXISTS "Directors can manage medical leaves" ON public.medical_leaves;
...
DROP POLICY IF EXISTS "Users can view medical leaves in their team" ON public.medical_leaves;
...
DROP POLICY IF EXISTS "System can manage capacity alerts" ON public.team_capacity_alerts;
...
DROP POLICY IF EXISTS "Managers and directors can view special approvals" ON public.special_approvals;
...
DROP POLICY IF EXISTS "Managers can create special approvals" ON public.special_approvals;
...
DROP TRIGGER IF EXISTS update_medical_leaves_updated_at ON public.medical_leaves;
...
CREATE INDEX IF NOT EXISTS idx_medical_leaves_person_dates ON public.medical_leaves (person_id, start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_medical_leaves_active ON public.medical_leaves (status, start_date, end_date) WHERE status = 'ATIVA';
CREATE INDEX IF NOT EXISTS idx_team_capacity_alerts_team_period ON public.team_capacity_alerts (team_id, period_start, period_end);
CREATE INDEX IF NOT EXISTS idx_special_approvals_request ON public.special_approvals (request_id);
CREATE INDEX IF NOT EXISTS idx_special_approvals_medical_leave ON public.special_approvals (medical_leave_id);