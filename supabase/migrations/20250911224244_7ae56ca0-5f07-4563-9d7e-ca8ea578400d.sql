-- Segregar papéis organizacionais de permissões administrativas (versão corrigida v4)
...
-- Recriar políticas RLS para requests (baseadas em organizational_role com cast)
...
USING (
  ...
  (EXISTS (
    ...
    WHERE prof.user_id = auth.uid() 
    AND per.organizational_role = 'DIRETOR'::organizational_role_type
  ))
  OR
